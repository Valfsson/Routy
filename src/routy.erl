...
router(Name, N, Hist, Intf, Table, Map) ->
  receive
    ...
    %% msg arrived to final destination
       {route, Name, From, Message} ->
        io:format("~w: received message ~w ~n", [Name, Message]),
        router(Name, N, Hist, Intf, Table, Map);

    %% forwards messages , or drops if the receiver's not found
    ...
    %% routing of a msg without knowing local routers name
      {send, To, Message} ->
        self() ! {route, To, Name, Message},
        io:format("~w: I got it! (~w)", [Name, Message]),
        router(Name, N, Hist, Intf, Table, Map);
